...
public class ImageProcessor {
    ...
    private static void bfs(int backgroundColor) {
        int height = pixelsArray.length;
        int width = pixelsArray[0].length;
        MyArrayList<int[]> turn = new MyArrayList<>();                                                                  // ArrayList
    ...
    private static int backgroundColor(int[][] pixelsArray) {
        ...
        int height = pixelsArray.length;
        int width = pixelsArray[0].length;
        MyArrayList<Integer> backgroundColors = new MyArrayList<>();                                                    // ArrayList
        ...
        for (int i = 0; i < backgroundColors.size(); i++) {
            ...
            for (int j = 0; j < backgroundColors.size(); j++) {
                if (backgroundColors.get(i) == backgroundColors.get(j)){
                    result++;
                }
            }
    ...
    private static void initializationOfVariables() {
        ...
        uncountableObjects = (pixelsArray.length * pixelsArray[0].length) * 0.01;
        // objects less than 5 percent of the average height and width are cropped.
        separationLength = (int) ((pixelsArray.length + pixelsArray[0].length) / 2 * 0.005);
    }
