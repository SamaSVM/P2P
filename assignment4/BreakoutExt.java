...
public class BreakoutExt extends WindowProgram {
     ...
     * A method that runs all the elements of the game in turn.
     */
    private void addTrialCounter() {
        counter = new GLabel("attempts : " + trialCounter ,getWidth() - 120, 20);
        counter.setFont("Verdana-15");
        add(counter);
    }
    ...
    private void addBricks() {
        for (int i = 0; i < NUMBER_OF_BRICKS_ROWS; i++) {
    ...
    private void bounceBall(GOval myBall) {
        ...
        while (true) {
            ...
            if (collider != myPaddle && collider != null && collider != counter) {
                if (cheat == 123){
                    vy = -vy;
                }
                brickCounter--;
            ...
            if (collider == myPaddle) {
                double bias = myBall.getX() - (myPaddle.getX() + (PADDLE_WIDTH/2));
                if (bias  <= -30)
                    vx = -3;
                else if (bias <=-20)
                    vx = -2;
            ...
            if (ballBelowFloor(myBall)) {
                addLabel("Next try!");
                if (brickCounter == NUMBER_OF_BRICKS_ROWS * NUMBER_OF_BRICKS_ROWS) {
                    cheat = readInt(":");
                }
                remove(myPaddle);
                remove(myBall);
